...
- **Phase 1**：使用 CustomTrainer + ClusterTrainingRuntime（纯 YAML，不修改 Go 代码），快速验证可行性
...
### 解决方案：在 Runtime command 中桥接环境变量

在 node 容器的 `command` 中使用 `/bin/sh -c`，将 `PET_*` 环境变量映射到 LLaMA Factory 期望的变量名：
...
### Step 1：创建 ClusterTrainingRuntime
...
> - `dataset_dir: /config`：用于让 LLaMA Factory 从 `/config/dataset_info.json` 读取数据集注册信息，`file_name` 再指向 `/workspace/dataset/data`
> - `finetuning_type: lora`：LoRA 微调，显存需求低
> - `ddp_timeout: 180000000`：分布式训练超时设置，避免节点启动时延导致超时
> - `trainer.numNodes: 1`：demo 默认用单节点单 GPU 验证，可按需直接修改同文件中的 TrainJob 段
...
| `examples/llamafactory/lora-sft-demo.yaml` | **新增** | 单文件 demo，包含训练配置 ConfigMap 与 TrainJob |
